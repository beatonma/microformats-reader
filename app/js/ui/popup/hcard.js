class HCard extends HObject {
    ...
    get() {
        ...
        card
            .addPrefix(this.getCardIcon(this.factory.get('name')))
    ...
    getIdentity(factory) {
        const display =
            new DivBuilder('card-section')
                .setSeparator(' ')
                .addPrefix(
                    new DivBuilder('h-section-label')
                        .add(getMessage('hcard_section_identity')))
        ...
        const extended =
            new DivBuilder()
                ...
                .add(factory.make('div', 'gender-identity', 'hcard_identity_gender'));
    ...
    getLinks(factory) {
        const display =
            new DivBuilder('card-section')
                .add(new DivBuilder('h-section-label').add(getMessage('hcard_section_links')))
    ...
    getLocation(factory) {
        const display =
            new DivBuilder('card-section')
                ...
                .addPrefix(
                        new DivBuilder('h-section-label')
                            .add(getMessage('hcard_section_location')))
    ...
    getOtherStuff(factory) {
        const display =
            new DivBuilder('card-section')
                .addPrefix(new SpanBuilder('h-section-label').add(getMessage('hcard_section_other')));

        const extended = new DivBuilder();
        const date = factory.get('bday');
        const bday =
            factory.make('span', 'bday', 'svg_icon_cake')
                .addSuffix(
                    new SpanBuilder()
                        .add(
                            format(' ({} {})',
        ...
        extended.add(
            new DivBuilder()
                .addPrefix(new SpanBuilder('h-item-label').add(format('{}: ', getMessage('hcard_other_publickey'))))
    ...
    getAffiliations(factory) {
        const display =
            new DivBuilder('card-section')
                .addPrefix(new DivBuilder('h-section-label').add(getMessage('hcard_section_affiliation')));
        ...
        for (let i = 0; i < json_hcard_list.length; i++) {
            ...
            if (image) {
                card.add(
                    new DivBuilder('hcard-image affiliate ' + image['classes'])
                        .allowEmpty()
                        .addStyle(format("background-image:url('{}');", image['value'])));
            }
            else {
                card.add(new SvgBuilder('hcard-image affiliate').setIcon('svg_icon_work'))
            }
            ...
            if (jobTitle) {
                textContainer.add(
                    new SpanBuilder()
                        .add(
                            new SpanBuilder(jobTitle.classes).add(jobTitle.value)
                        )
                        .addSuffix(orgName ? format(' {} ', getMessage('role_at_organisation_preposition')) : ''));
            }
    ...
    buildSimpleAffiliate(factory) {
        ...
        const card =
            new DivBuilder('hcard-affiliate flex-horizontal')
                .add(new SvgBuilder('hcard-image affiliate icon').setIcon('svg_icon_work'));
